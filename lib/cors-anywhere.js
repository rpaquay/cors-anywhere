...
function proxyRequest(req, res, proxy) {
  // The location returned from 'url.parse'
  // See https://nodejs.org/docs/latest/api/url.html
  var location = req.corsAnywhereRequestState.location;

  // If there is an 'username:password' component in the url, remove it from the
  // url and add a 'Authorization' header (i.e. do what Web Browsers do
  // implicitly).
  var auth = location.auth;
  if (auth) {
    var authBase64 = new Buffer(auth).toString('base64');
    req.headers['Authorization'] = 'Basic ' + authBase64;
    delete location.auth;
    location = url.parse(url.format(location));
  }

  req.url = location.href;
  // Let the "Host" header be the host part of the path (including port, if
  // specified).
  req.headers.host = location.host;

  // Start proxying the request
  if (auth) {
    console.log('Proxying request (with auth): ' + req.url);
  } else {
...
function onProxyResponse(response, req, res) {
  ...
  console.log('Process response: ' +  req.url + ' : ' + statusCode);
